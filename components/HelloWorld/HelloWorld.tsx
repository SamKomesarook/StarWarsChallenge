...
export function HelloWorld() {
  ...
  const [getStarship, { loading, data }] = useLazyQuery(GET_STARSHIP, {onCompleted: (data) => {
    setSelected(data)
  }});

  const selectShip = async (ship) => {
    setSelected(ship);
    getStarship({variables:{
      id:ship.id
    }})
  }
  ...
  const handleChange = (event) => {
    setFiltered(starships.filter(starship => {
  ...
  return (<div style={{display: 'flex', flexDirection: 'row'}}>
    ...
    <Stack spacing={8}>
    {
      filtered!== [] && filtered.map((elem: any) => {
        return <Feature
        title={elem.name}
        desc={elem.hyperdriveRating}
        item={elem}
      />
      })
    }
    ...
    <div style={{flex:2}}>
    <div style={{backgroundColor: '#E6F0EF', height:'50vh', marginTop:'25vh', alignContent: 'center', alignItems:'center', display:'flex', textAlign: 'center', flexDirection:'column'}}>
      {
        !data && !loading && <>
          Select something...
        </>
